...
* Last modified by:
* Nick Sorrell
* ... to make changes allowing the page to run at 60-fps and be jank-free.
*/

// establish arrays for all possible pizza ingredients used by generator functions
...
* - Takes a string parameter and returns an array of strings based on
*   the supplies parameter string.
*/
function getAdj(x) {
    switch (x) {
        case "dark":
...
* - Takes two string parameters, which are fed into getAdj() and getNoun()
    then chooses a random result from the returned string arrays
    finally returning a concatenated string with the chosen words
*/
...
* returns a DOM element for each pizza
  - Take a number parameter, which is used to assign an id to the
  generated pizza.
*/
var pizzaElementGenerator = function(i) {
    ...
    // assign dimensions, unique ID, and bootstrap class to pizza containers
    ...
    // add response pizza image to each image container
    pizzaImage.src = "images/pizza.png";
    pizzaImage.classList.add("img-responsive");
    pizzaImageContainer.appendChild(pizzaImage);
    pizzaContainer.appendChild(pizzaImageContainer);

    // fetch a random name and append it to the description container
    pizzaDescriptionContainer.classList.add("col-md-6"); //bootstrap class
    pizzaName = document.createElement("h4");
    pizzaName.innerHTML = randomName();
    pizzaDescriptionContainer.appendChild(pizzaName);

    // fetch a list of ingredients and append them to the description container
...
* - Takes a number parameter which is passed to changeSliderLabel and changePizzaSizes functions
*
* - Note: The dx function was removed because it is not necessary. It was instead replaced with the switch
*   in changePizzaSizes() to keep things simple
*/
...
* - Takes a number parameter which is used to select the new width for the pizzas
*/
function changePizzaSizes(size) {
    'use strict';
    var newwidth = 100;
    //Decide which width percentage value to use based on parameter
    switch (size) {
        case "1":
            newwidth = 25;
            break;
    ...
    //Look up all random pizza containers for style changes in the loop
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer");
    for (var i = 0, j = randomPizzas.length; i < j; i++) {
        randomPizzas[i].style.width = newwidth + "%"; //simply apply the style change for width
    }
}

/*
* Changes the value for the size of the pizza above the slider
* - Takes a number parameter which is used to select the text for the pizza size label
*/
...
// create a document fragement to hold all of our generated pizzas
var pizzasFragment = document.createDocumentFragment();

// generate all of our pizzas and append them to the fragment
for (var i = 2; i < 100; i++) {
    pizzasFragment.appendChild(pizzaElementGenerator(i));
}

// append the fragment to the randomPizzas div
